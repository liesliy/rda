...
2. **Critical hard failures -> EXCLUDE**; three-level classification
   (D-14: joint_limit can be PASS / REVIEW / EXCLUDE depending on severity).
...
def test_joint_limit_three_level_classification(catalog):
    """D-14: joint_limit uses three-level classification.

    - Actual violation (value > limit) -> EXCLUDE
    - Approaching boundary (within 2%) -> REVIEW
    """
    _, results = catalog

    # EXCLUDE: actual violation
    if "joint_limit_violation" in results:
        res = results["joint_limit_violation"]
        assert res.verdict == AuditVerdict.EXCLUDE, (
            f"joint_limit_violation should be EXCLUDE, got {res.verdict}"
        )
        assert "joint_limit" in _fired(res)

    # REVIEW: approaching boundary
    if "joint_limit_approaching" in results:
        res = results["joint_limit_approaching"]
        assert res.verdict == AuditVerdict.REVIEW, (
            f"joint_limit_approaching should be REVIEW, got {res.verdict}"
        )
        assert "joint_limit" in _fired(res)
